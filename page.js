...
const Statistics = () => {
  const caseStudies = [
    ...
    { id: 2, tag: "Environment and Sustainability", publishNumber: "5", popularity: "20%", trimester: "2" },
    ...
    { id: 5, tag: "Environment and Sustainability", publishNumber: "5", popularity: "20%", trimester: "3" },
    { id: 6, tag: "Business and activity", publishNumber: "8", popularity: "90%", trimester: "2" },
    { id: 7, tag: "Safety and Well-being", publishNumber: "4", popularity: "11%", trimester: "2" },
    { id: 8, tag: "Environment and Sustainability", publishNumber: "5", popularity: "20%", trimester: "1" },
    { id: 9, tag: "Business and activity", publishNumber: "8", popularity: "90%", trimester: "2" },
  ];
  ...
  return (
    ...
    >
      <Header />
      <h1 className="text-7xl font-bold px-[2rem] pt-[1rem] pb-[4rem]">{t("Statistics")}</h1>
      <div className="flex">
      ...
      <main style={{ flex: "1 0 auto", width: "100%" }}>
        <div style={{ padding: "0 50px" }}>
          <section aria-label="Statistics section">
            <select value={tagFilter} onChange={(e) => setTagFilter(e.target.value)} className="p-2 m-2 border shadow-lg">
            ...
            <select value={publishFilter} onChange={(e) => setPublishFilter(e.target.value)} className="p-2 m-2 border shadow-lg">
            ...
            <select value={popularityFilter} onChange={(e) => setPopularityFilter(e.target.value)} className="p-2 m-2 border shadow-lg">
            ...
            <div className="flex">
              <div className="border-solid bg-white shadow-2xl border-2 border-black-600 py-8 px-10 my-10">
                <h2 className="text-2xl font-bold text-gray-400">{t("Total Results")}</h2>
                <p className="text-[1.8rem] font-bold text-center pt-[15px] text-black-400">{filteredStudies.length}</p>
              </div>
            </div>
            ...
            <div className="overflow-hidden rounded-lg shadow">
              ...
              <table className="min-w-full bg-white">
                <thead>
                  <tr>
                    <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider rounded-tl-lg">{t("ID")}</th>
                    <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t("Tag")}</th>
                    <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider">{t("number")}</th>
                    <th className="px-5 py-3 border-b-2 border-gray-200 bg-gray-100 text-left text-xs font-semibold text-gray-600 uppercase tracking-wider rounded-tr-lg">{t("Popularity")}</th>
                  </tr>
                </thead>
                <tbody>
                  {records
                    .filter((item) => serach.toLowerCase() === "" || item.tag.toLowerCase().includes(serach))
                    .map((study, index) => (
            ...
            <nav className="bg-gray-200 py-3">
              <ul className="pagei">
                <li className="page-item">
                  <button className="float-right text-black font-bold py-1 px-2" onClick={nextPage}>{">"}</button>
                  <p className="float-right text-black py-1 px-2">{currentPage}</p>
                </li>
                <li className="page-item">
                  <button className="float-right text-black font-bold py-1 px-2" onClick={prePage}>{"<"}</button>
                </li>
                ...
                <li>
                  <p>{firstindex + 1} - {Math.min(lastindex, filteredStudies.length)} of {filteredStudies.length}</p>
                </li>
              </ul>
            </nav>
          </section>
